...
import { OnboardingContract, Employee, Candidate, Job, EmployeeBenefit, TimeRegister, PaySlip, DocumentFile } from '../types';
...
export default function OnboardingModule({
  ...
  const handleImportCandidate = (cand: Candidate) => {
    setEmpName(cand.name);
    setEmpEmail(cand.email);
    setEmpPhone(cand.phone);
    setEmpRole(cand.jobTitle || cand.area || 'Desenvolvedor / Especialista');
    setEmpDept(cand.area || 'TI');
    setEmpSalary('5000');
    setIsImportCandidateOpen(false);
    setIsNewContractOpen(true);
    triggerToast(`✓ Dados do candidato ${cand.name} importados para o formulário!`);
  };
        ...
        const updatedDocs = c.checklistDocuments.map(d => {
          if (d.id === docId) {
            return {
              ...d,
              status: newStatus,
              ...(newStatus === 'Recebido' ? { fileName: `${d.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}_upload.pdf`, uploadedAt: new Date().toISOString().split('T')[0] } : {}),
  ...
  const handleCompleteOnboarding = (contract: OnboardingContract) => {
    ...
    // Provision default benefits linked to new employee
    if (onUpdateEmployeeBenefits) {
      const defaultBenefits: EmployeeBenefit[] = [
        { id: `eb-vt-${Date.now()}`, employeeId: newEmpId, employeeName: contract.employeeName, benefitId: 'b1', benefitName: 'Vale Transporte (VT)', benefitType: 'VT', category: 'Provento', valueType: 'Fixo', value: 220, status: 'Ativo' },
        { id: `eb-vr-${Date.now()}`, employeeId: newEmpId, employeeName: contract.employeeName, benefitId: 'b2', benefitName: 'Vale Refeição (VR)', benefitType: 'VR', category: 'Provento', valueType: 'Fixo', value: 750, status: 'Ativo' },
        { id: `eb-saude-${Date.now()}`, employeeId: newEmpId, employeeName: contract.employeeName, benefitId: 'b3', benefitName: 'Plano de Saúde Unimed', benefitType: 'Saúde', category: 'Provento', valueType: 'Fixo', value: 450, status: 'Ativo' }
      ];
      onUpdateEmployeeBenefits([...employeeBenefits, ...defaultBenefits]);
    }
    ...
    if (updatedSelected) setSelectedContract(updatedSelected);
    triggerToast(`✓ Colaborador(a) ${contract.employeeName} efetivado(a) com sucesso em GestRH e vinculado a todos os benefícios & folha!`);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col h-[650px]">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-1">
              <button 
                onClick={() => setIsImportCandidateOpen(true)}
                className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-xl transition-all flex items-center space-x-1.5 text-xs font-semibold cursor-pointer"
                title="Importar do Banco de Talentos"
              >
                <Users className="h-4 w-4" />
                <span className="hidden sm:inline">Importar Candidato</span>
              </button>
              <button 
                onClick={() => setIsNewContractOpen(true)}
                className="bg-emerald-600 hover:bg-emerald-500 text-white p-2 rounded-xl transition-all flex items-center space-x-1 cursor-pointer shadow-sm"
                title="Nova Contratação Manual"
              >
      ...
      {/* Modal: Importar Candidato do Banco de Talentos / Recrutamento */}
      {isImportCandidateOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 space-y-6 shadow-2xl border border-slate-100">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div>
                <h3 className="font-display font-bold text-lg text-slate-900 flex items-center space-x-2">
                  ...
                  <span>Importar Candidato do Banco / Recrutamento</span>
                </h3>
                <p className="text-xs text-slate-500 mt-1">Selecione um candidato cadastrado para preencher automaticamente a admissão</p>
              </div>
              <button 
                onClick={() => setIsImportCandidateOpen(false)}
                className="text-slate-400 hover:text-slate-600 p-2 rounded-xl transition-all cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="relative">
              ...
              <input 
                type="text"
                placeholder="Buscar candidato por nome, cargo ou área..."
                value={candidateSearchTerm}
                onChange={(e) => setCandidateSearchTerm(e.target.value)}
                className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium"
              />
            </div>
            ...
            <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
              {candidates.filter(c => {
                ...
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <p className="text-xs font-semibold text-slate-500">Nenhum candidato encontrado no Banco de Talentos.</p>
                </div>
              ) : (
                candidates
                  .filter(c => {
                  ...
                  .map(cand => (
                    <div 
                      key={cand.id}
                      className="p-4 rounded-2xl border border-slate-100 hover:border-emerald-300 bg-slate-50 hover:bg-emerald-50/30 transition-all flex items-center justify-between"
                    >
                      <div>
                        <div className="font-bold text-slate-800 text-sm">{cand.name}</div>
                        <div className="text-xs text-slate-500 flex items-center space-x-2 mt-0.5">
                          <span>{cand.jobTitle || cand.area || 'Desenvolvedor / Especialista'}</span>
                          <span>&bull;</span>
                      ...
                      <button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2 rounded-xl transition-all flex items-center space-x-1 cursor-pointer"
                      >
            ...
            <div className="pt-3 border-t border-slate-100 flex justify-end">
              <button
                type="button"
                onClick={() => setIsImportCandidateOpen(false)}
                className="bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold text-xs px-4 py-2 rounded-xl cursor-pointer"
              >
                Fechar
              </button>
            </div>
