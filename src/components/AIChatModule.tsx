...
*Selecione uma das perguntas rápidas abaixo para começar ou digite sua dúvida no campo de texto!*`
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col h-[650px] shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto py-6 space-y-4 pr-1">
        {messages.map((msg, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`flex items-start space-x-3 max-w-3xl ${isAI ? 'mr-12' : 'ml-auto flex-row-reverse mr-0 pl-12'}`}
            >
              {/* Avatar Icon */}
              <div className={`h-8 w-8 rounded-xl shrink-0 flex items-center justify-center font-bold text-xs ${isAI ? 'bg-slate-900 text-emerald-400' : 'bg-emerald-600 text-white'}`}>
                {isAI ? <Bot className="h-4.5 w-4.5" /> : 'U'}
              </div>

              {/* Text Bubble */}
              <div className={`p-4 rounded-2xl ${isAI ? 'bg-slate-50 border border-slate-100' : 'bg-[#0b1d33] text-white'}`}>
                {isAI ? (
        ...
        {isLoading && (
          <div className="flex items-start space-x-3 max-w-3xl mr-12">
            <div className="h-8 w-8 bg-slate-900 text-emerald-400 rounded-xl shrink-0 flex items-center justify-center">
              <Bot className="h-4.5 w-4.5" />
            </div>
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center space-x-2">
      ...
      <div className="py-3 border-t border-slate-100 flex flex-wrap gap-2 overflow-x-auto shrink-0">
        {QUICK_PROMPTS.map((prompt, i) => (
          <button
            key={i}
            onClick={() => handleSendMessage(prompt.text)}
            disabled={isLoading}
            className="bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-200 text-slate-600 hover:text-emerald-800 text-[10px] font-bold py-1.5 px-3 rounded-xl transition-all cursor-pointer inline-flex items-center space-x-1"
          >
      ...
      <form 
        ...
        className="pt-3 border-t border-slate-100 flex items-center space-x-2 shrink-0"
      >
        <input 
          type="text"
          placeholder="Pergunte sobre regras da CLT, contratações, demissões ou peça para redigir uma vaga..."
          value={inputText}
          onChange={(e) => setInputText(e.target.value)}
          disabled={isLoading}
          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-4 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium"
        />
        <button
          type="submit"
          disabled={isLoading || !inputText.trim()}
          className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-200 text-white p-2.5 rounded-xl transition-all flex items-center justify-center shrink-0 cursor-pointer shadow-md shadow-emerald-600/10"
        >
